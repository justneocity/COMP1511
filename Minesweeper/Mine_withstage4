...
int on_border (int row, int column);
    ...
    int gameModeOn = FALSE;
     ...
     while (gameover == 0 && scanf("%d", &command) == 1) {
        if (command == DETECT_ROW) {
            ...
            reveal_radial(minefield, &gameover, mines);
        }
...
void reveal_radial(int minefield[SIZE][SIZE], int *gameover, int mines) {
    ...
    int direction = 0; //this will determine if reveal still going
    
    int mines_radial = detect_gameplay(row, column, minefield);
    ...
    //direction will increment from 0 - 7 representing points on the compass
    while (direction != END) {
        int currentRow = row;
        int currentColumn = column;
        int finish = FALSE;
        //check and decide to reveal or not reval
        while (finish == FALSE) { 
        //Check if coordinate on edge, if so finish = TRUE;
        finish = on_border(currentRow, currentColumn);
        ...
        if (finish == FALSE) {
            ...
            int mine_radial = detect_gameplay(currentRow, currentColumn, minefield);
